...
echo -e "${CYAN}Highest Occurring Primers Across PRJNA Datasets${RESET}"
echo "Highest Occurring Primers Across PRJNA Datasets" > "$summary_all"
echo "================================================" >> "$summary_all"
...
for dir in PRJNA*; do
    if [[ -d $dir ]]; then
        ...
        echo "Primer Detection Results" > "$output_file"
        echo "=========================" >> "$output_file"
        echo "Detected Primer Summary" > "$summary_file"
        echo "========================" >> "$summary_file"

        # Initialize highest primer tracking variables
        highest_count_forward=0
        highest_primer_forward=""
        highest_primer_seq_forward=""

        highest_count_reverse=0
        highest_primer_reverse=""
        highest_primer_seq_reverse=""
        ...
        for file in "$dir"/*.fastq.gz; do
            ...
            # Update forward or reverse primer stats
            if [[ $file == *_1.fastq.gz ]]; then
                if [[ $highest_count -gt $highest_count_forward ]]; then
                    highest_count_forward=$highest_count
                    highest_primer_forward=$highest_primer
                    highest_primer_seq_forward=$highest_primer_seq
                fi
            elif [[ $file == *_2.fastq.gz ]]; then
                if [[ $highest_count -gt $highest_count_reverse ]]; then
                    highest_count_reverse=$highest_count
                    highest_primer_reverse=$highest_primer
                    highest_primer_seq_reverse=$highest_primer_seq
                fi
            fi
        ...
        echo -e "\n$dir:" >> "$summary_all"
        if [[ -z $highest_primer_forward && -z $highest_primer_reverse ]]; then
            echo "No primers detected." >> "$summary_all"
        else
            echo "Highest occurring primer (Forward Reads): $highest_primer_forward ($highest_count_forward occurrences)" >> "$summary_all"
            echo "Highest occurring primer (Reverse Reads): $highest_primer_reverse ($highest_count_reverse occurrences)" >> "$summary_all"
        fi
